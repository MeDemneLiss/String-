...
s21_string.a: repo $(OBJECTS)
	ar rcs s21_string.a $(OBJ)/*.o
	ranlib s21_string.a
	
test: reset s21_string.a 
	$(CC) $(FLAGS) test_s21/*.c *.c s21_string.a $(TEST_LIBS) -o s21_test
	./$(TEST)
...
fix:
	...
	clang-format -n *.c *.h test_s21/*.c test_s21/*.h test_db/*.c
	clang-format -i *.c *.h test_s21/*.c test_s21/*.h test_db/*.c
	sleep 1
	reset
	clang-format -n *.c *.h test_s21/*.c test_s21/*.h test_db/*.c
...
reset:
 ...
 git:
	git add .
	git commit -m "$(DATE)"
# git push origin develop_ogdengum
